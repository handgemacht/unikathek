...
urlParams.get('model') ? primaryKey=urlParams.get('model') : primaryKey=false;
(urlParams.get('history') === 'true') ? hasHistory=true : hasHistory=false;
(urlParams.get('from') === 'true') ? hasFrom=true : hasFrom=false;
...
function loadModelViewer(modelJSON) {
	...
	const annotationContainer = app.modelViewer.annotations.containerEl;

	app.dev && console.log('dev --- modelJSON: ', modelJSON);
...
function loadContent(modelJSON, modelViewer, annotationContainer) {
	...
	//set title
	app.gui.title.set(modelJSON.basicData.name, true, hasHistory, modelJSON.primaryKey)
	...
	function createAnnotations (modelJSON) {
		...
		for(let annotation of modelJSON.appData.annotations){
			...
			if(annotation.mediaType === 'read'){
				hotspotIcon = '<img src="assets/hand.gemacht WebApp icon read pearlwhite.svg" alt="Interaktiver Punkt am 3D Modell" class="hotspot-icon" width="100px" height="100px">';
			}else if(annotation.mediaType === "listen"){
				hotspotIcon = '<img src="assets/hand.gemacht WebApp icon listen pearlwhite.svg" alt="Interaktiver Punkt am 3D Modell" class="hotspot-icon" width="100px" height="100px">';
			}else if(annotation.mediaType === "watch"){
				hotspotIcon = '<img src="assets/hand.gemacht WebApp icon watch pearlwhite.svg" alt="Interaktiver Punkt am 3D Modell" class="hotspot-icon" width="100px" height="100px">';
			}else if(annotation.mediaType === "move"){
				hotspotIcon = '<img src="assets/hand.gemacht WebApp icon move pearlwhite.svg" alt="Interaktiver Punkt am 3D Modell" class="hotspot-icon" width="100px" height="100px">';
			}
			...
			for(let annotationContent of annotationContentHTMLsArray) {
				if(annotationContent.type === 'headline'){
					...
					+ 'src="' + app.filepaths.files + 'annotation-media/' + annotationContent.filename + '" ' 
					...
					annotationContentHTML = annotationContentHTML.concat('<audio controls><source src="' + app.filepaths.files + 'annotation-media/' + annotationContent.filename + '" type="audio/mpeg"></audio>');
			...
			+ 'data-focus="false">' 
			+ '<div class="annotation-close-container"><img src="assets/hand.gemacht WebApp icon small close coalgrey.svg" alt="Schließen-Icon" class="annotation-close-symbol" width="100px" height="100px"></div>' 
