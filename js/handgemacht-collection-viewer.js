			...
			let objectsJSON = fetch(fileJSON)
				...
				.then((json) => {
					...
					newEntity.setAttribute('forcegraph', {
						...
						onLinkHover: link => { 
							document.querySelector("#forcegraph-tooltip").setAttribute("value", link ? link.name : ""); 
						},
						onNodeHover: node => { 
							document.querySelector("#forcegraph-tooltip").setAttribute("value", node ? node.name : ""); 
						},
						onNodeClick: node => { 
...
AFRAME.registerComponent('camera-focus-target', {
	...
	init: function () {
		...
		let focusEl = document.createElement('a-entity');
		let focus = focusEl.object3D;
		let focusPosStr = this.camera.position.x + ' ' + this.camera.position.y + ' ' + this.camera.position.z;
		focusEl.setAttribute('position', focusPosStr);
		this.el.sceneEl.appendChild(focusEl);

		//get vector to target and rotate focus
		this.targetVector = this.lookAtVector(this.camera, this.target);
		let targetVector = this.targetVector;
		focus.lookAt(this.targetVector);

		//set new target position in front of object
		let newTargetPosition = new THREE.Vector3(); 
		this.target.getWorldPosition(newTargetPosition);
		newTargetPosition.sub(targetVector.setLength(this.data.distance).negate());

		//position values
		let cameraTargetPosX = newTargetPosition.x;
		let cameraTargetPosY = newTargetPosition.y;
		let cameraTargetPosZ = newTargetPosition.z;
		...
		//rotation values
		let focusRotX = (focus.rotation.x*180)/Math.PI;
		let focusRotY = (focus.rotation.y*180)/Math.PI;
		let focusRotZ = 0;
		...
		this.cameraEl.addEventListener('animationcomplete__cptt-z', (e) => {
			document.querySelector('a-camera').removeAttribute('animation__cmtt-x');
			document.querySelector('a-camera').removeAttribute('animation__cmtt-y');
			document.querySelector('a-camera').removeAttribute('animation__cmtt-z');
			document.querySelector('a-camera').removeAttribute('animation__cptt-x');
			document.querySelector('a-camera').removeAttribute('animation__cptt-y');
			document.querySelector('a-camera').removeAttribute('animation__cptt-z');
			devMode && console.log('dev --- animationcomplete', e);
			this.cameraEl.removeAttribute('camera-focus-target');
		});
	},
	...
	remove: function () {
		...
		this.el.object3D.getWorldPosition(cameraWorldPosition);

		this.cameraEl.setAttribute('look-controls', {enabled: true, orientation: {'position': cameraWorldPosition, 'rotation': this.camera.rotation}});
